...
RUN set -ex \
    ...
    && pip install pyasn1 \
    && pip install apache-airflow[crypto,celery,postgres,hive,jdbc,mysql,ssh,google]==${AIRFLOW_VERSION} --constraint ${CONSTRAINT_URL}\
...
RUN git clone https://github.com/andreax79/airflow-code-editor.git \
